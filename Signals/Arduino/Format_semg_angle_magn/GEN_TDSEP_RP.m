% Generate dataset w/ TDSEP processing %
...
out_file_prepend_list = {'TR_TDSEP_', 'CV_TDSEP_', 'TS_TDSEP_'};
...
% Output dataset for LSTM: R for Roll, P for Pitch

% Roll
train_out_name_R = [out_file_prepend_list{1}, num2str(exp_num), out_filename, '_R'];  
train_out_file_R = [out_file_loc_prepend, train_out_name_R out_file_extension];  
generate_LSTM_data_mpuIdx(train_out_file_R, processed_join_dataset{1}, 1);

cv_out_name_R = [out_file_prepend_list{2}, num2str(exp_num), out_filename, '_R'];  
cv_out_file_R = [out_file_loc_prepend, cv_out_name_R out_file_extension];  
generate_LSTM_data_mpuIdx(cv_out_file_R, processed_join_dataset{2}, 1);

test_out_name_R = [out_file_prepend_list{3}, num2str(exp_num), out_filename, '_R'];  
test_out_file_R = [out_file_loc_prepend, test_out_name_R out_file_extension];    
generate_LSTM_data_mpuIdx(test_out_file_R, processed_join_dataset{3}, 1);

% Pitch
train_out_name_P = [out_file_prepend_list{1}, num2str(exp_num), out_filename, '_P'];  
train_out_file_P = [out_file_loc_prepend, train_out_name_P out_file_extension];  
generate_LSTM_data_mpuIdx(train_out_file_P, processed_join_dataset{1}, 2);

cv_out_name_P = [out_file_prepend_list{2}, num2str(exp_num), out_filename, '_P'];  
cv_out_file_P = [out_file_loc_prepend, cv_out_name_P out_file_extension];  
generate_LSTM_data_mpuIdx(cv_out_file_P, processed_join_dataset{2}, 2);

test_out_name_P = [out_file_prepend_list{3}, num2str(exp_num), out_filename, '_P'];  
test_out_file_P = [out_file_loc_prepend, test_out_name_P out_file_extension];    
...
fprintf(['final float [] semg_normalization_min = {', ...
        char(strjoin(string(num2str(semg_min_value)), ',')), '};', '\n']);

invC = inv(C);
fprintf('final float [][] TDSEP_matrix = \n{');
for i = 1 : semg_channel_count
    temp_str = sprintf('%f,', invC(i, :)); 
    temp_str = temp_str(1:end-1);
    if i ~= semg_channel_count
        fprintf(['{', temp_str, '},', '\n']);
    else
        fprintf(['{', temp_str, '}']);
    end
...
%% Show result
% temp_RMS_list_R = verify_multi_semg(test_out_name_R);
% temp_RMS_list_P = verify_multi_semg(test_out_name_P);
% fprintf('===========\n');
% for i = 1 : num_of_gesture    
%     for r = 1 : test_size        
