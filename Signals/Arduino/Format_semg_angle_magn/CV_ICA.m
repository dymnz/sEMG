...
% [pica_semg, pW, W] = fastica(semg, ...
%     'verbose', 'off', 'displayMode', 'off');  

tdsep_tau = [0:2];
C = tdsep2(semg, tdsep_tau);
pica_semg = C \ semg;

% RMS
rms_semg = RMS_calc(semg, RMS_window_size);
ica_semg = RMS_calc(pica_semg, RMS_window_size);
...
%  figure;
%  subplot_helper(1:length(pica_semg), pica_semg(2, :), ...
%                 [semg_channel_count 1 1], ...
%                 {'sample' 'amplitude' ['Fast-ICA sEMG ch:'  num2str(channel)]}, '-');    
% ylim([min(min(pica_semg)) max(max(pica_semg))]);
%  subplot_helper(1:length(pica_semg), pica_semg(3, :), ...
%                 [semg_channel_count 1 2], ...
%                 {'sample' 'amplitude' ['Fast-ICA sEMG ch:'  num2str(channel)]}, '-');    
% ylim([min(min(pica_semg)) max(max(pica_semg))]);
%  subplot_helper(1:length(pica_semg), pica_semg(1, :), ...
%                 [semg_channel_count 1 3], ...
%                 {'sample' 'amplitude' ['Fast-ICA sEMG ch:'  num2str(channel)]}, '-');    
% ylim([min(min(pica_semg)) max(max(pica_semg))]);
%  subplot_helper(1:length(pica_semg), pica_semg(4, :), ...
%                 [semg_channel_count 1 4], ...
%                 {'sample' 'amplitude' ['Fast-ICA sEMG ch:'  num2str(channel)]}, '-');    
% ylim([min(min(pica_semg)) max(max(pica_semg))]);
% 
%  figure;
...
%                 [semg_channel_count 1 1], ...
%                 {'sample' 'amplitude' ['Fast-ICA Demixed RMS sEMG ch:'  num2str(channel)]}, '-');    
...
% ylim([min(min(ica_semg)) max(max(ica_semg))]);
 ...
 subplot_helper(1:length(pica_semg), pica_semg(1, :), ...
                [semg_channel_count 1 1], ...
                {'sample' 'amplitude' ['TDSEP sEMG ch:'  num2str(channel)]}, '-');    
ylim([min(min(pica_semg)) max(max(pica_semg))]);
 subplot_helper(1:length(pica_semg), pica_semg(2, :), ...
                [semg_channel_count 1 2], ...
                {'sample' 'amplitude' ['TDSEP sEMG ch:'  num2str(channel)]}, '-');    
