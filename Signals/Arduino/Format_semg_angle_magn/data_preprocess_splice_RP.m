...
for f = 1 : length(file_label_list)
    ...
    for idx = 1 : length(mpu_segment_index_list)
        mpu_segments(abs(mpu(:, mpu_segment_index_list(idx) )) ...
            > mpu_segment_threshold) = 1;
    end
    

    figure;
    subplot_helper(1:length(mpu), mpu(:, mpu_segment_index_list(1)), ...
                    [2 1 1], {'sample' 'amplitude' 'Normalized angle'}, 'o');         
    subplot_helper(1:length(mpu_segments), mpu_max_value .* mpu_segments, ...
                    [2 1 1], {'sample' 'amplitude' 'Normalized sEMG'}, '-');                                                       
    subplot_helper(1:length(mpu), mpu(:, mpu_segment_index_list(2)), ...
                    [2 1 2], {'sample' 'amplitude' 'Normalized angle'}, 'o');         
    subplot_helper(1:length(mpu_segments), mpu_max_value .* mpu_segments, ...
                    [2 1 2], {'sample' 'amplitude' 'Normalized sEMG'}, '-');      
