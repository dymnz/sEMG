...
train_output_filename = './data/output/temp.txt';
test_output_filename = './data/output/exp_2_first_half_stream_rect_DS300_pulse.txt';
...
%% Pulse gen
threshold = 0.05;
processed_semg = zeros(size(semg));
for i = 2 : length(semg)
    if semg(i) > threshold && ...
       (semg(i) - semg(i-1)) * ...
       (semg(i - 1) - semg(i)) < 0
   
        processed_semg(i) = semg(i);
    end
end
semg = processed_semg;
figure;
