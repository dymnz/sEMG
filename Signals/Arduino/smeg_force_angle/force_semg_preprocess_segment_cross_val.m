...
train_output_filename = './data/output/exp_SFM_DS200_EVEN_1.txt';
...
%% Remove faulty data
usable_data_range = 1000 : min(length(semg), length(force));
...
output_segment_indices = ...
        2 : 2 : floor(length(mid_segment_indices) / 2); % EVEN
% output_segment_indices = ...
%         3 : 2 : floor(length(mid_segment_indices) / 2); % ODD
    
num_of_sample = length(output_segment_indices);
fprintf(output_fileID, '%d\n', num_of_sample);
for i = output_segment_indices
