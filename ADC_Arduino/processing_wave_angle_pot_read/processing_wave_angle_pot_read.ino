...
// Sends raw value, translation to angle will be done in Processing
const int pot_channel = 1;
const int pot_packet_byte = 2;  // int8_t for +-128 degree
const int pot_packet_len = alignment_packet_len + pot_channel * pot_packet_byte;

uint8_t semg_packet[semg_packet_len] = {'$'};
uint8_t mpu_packet[mpu_packet_len] = {'@'};
uint8_t pot_packet[mpu_packet_len] = {'%'};
...
void POT_init(struct POTData *pot) {
  SerialUSB.println("POT_init");
...
void POT_read(struct POTData *pot) {
  pot->data[pot->pointer] = analogRead(pot->pin);
  pot->pointer = (pot->pointer + 1) % MV_SIZE;
}

int POT_calculateAverage(struct POTData *pot) {
  float sum = 0;
  for (indexType i = 0; i < MV_SIZE; i++) {  
    sum += pot->data[i];
  }
  
  pot->average = sum / MV_SIZE;
}
