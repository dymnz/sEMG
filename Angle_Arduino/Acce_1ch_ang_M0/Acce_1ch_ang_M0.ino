...
void loop() {
  ...
  while (true) {
    ...
    estimate_angleX = ((float)moving_diff / MAX_VALUE) * (MAX_ANGLE - MIN_ANGLE) + MIN_ANGLE + ANGLE_SHIFT;
...
int readX(void) {
  ...
  return  (int16_t)(Wire.read() << 8 | Wire.read()); // 0x3D (ACCEL_YOUT_H) & 0x3E (ACCEL_YOUT_L)
}
